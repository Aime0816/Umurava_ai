...
export function ScreeningView() {
  ...
  const addManual = () => {
    ...
    const skills = manual.skills
      .split(',')
      .map((s) => ({
        name: s.trim(),
        level: 'Intermediate' as const,
        yearsOfExperience: parseInt(manual.yoe) || 2,
      }))
      .filter((s) => s.name);
    ...
    const candidate = {
      ...
      experience: [],
      education: [{ institution: 'N/A', degree: "Bachelor's", fieldOfStudy: 'N/A', startYear: 2016, endYear: 2020 }],
      projects: [],
    ...
    const samples = [
      {
        ...
        education: [{ institution: 'University of Rwanda', degree: "Bachelor's", fieldOfStudy: 'Computer Science', startYear: 2014, endYear: 2018 }],
        projects: [{ name: 'AI Chat System', description: 'LLM-powered assistant', technologies: ['Node.js', 'Gemini API'], role: 'Lead' }],
        ...
        experience: [
          {
            ...
            startDate: '2018-06',
            endDate: 'Present',
            description: 'Platform serving 2M users',
            technologies: ['Node.js', 'K8s'],
            isCurrent: true,
          },
        ],
        education: [{ institution: 'UCL', degree: "Master's", fieldOfStudy: 'Computer Science', startYear: 2014, endYear: 2016 }],
        projects: [{ name: 'Distributed Messaging', description: 'Kafka-based event system', technologies: ['Node.js', 'Kafka'], role: 'Architect' }],
        availability: { status: 'Open to Opportunities', type: 'Full-time' },
        certifications: [{ name: 'CKA', issuer: 'CNCF', issueDate: '2021-03' }],
        source: 'json',
      },
    ];
  ...
  const handleRunScreening = async () => {
    ...
    if (!staged.length) {
      toast.error('Add candidates before screening');
      return;
    }
    ...
    try {
      ...
      const screeningAction = await dispatch(
        runScreening({
          jobId: job._id,
  ...
  return (
    <div className="mx-auto w-full max-w-7xl p-4 sm:p-6 lg:p-7">
      <div className="mb-5 rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4 sm:p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <div className="text-[11px] uppercase tracking-widest text-white/35">Screening Form</div>
            <h2 className="font-serif text-3xl font-light tracking-tight text-white">Create and evaluate jobs from one clean workspace</h2>
            <p className="mt-1 text-sm text-white/45">Add job requirements, load candidates, and run screening without losing the flow on smaller screens.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button className="btn-ghost btn btn-sm" onClick={() => dispatch(setView('jobs'))}>Open Jobs</button>
            <button className="btn-primary btn btn-sm" onClick={() => dispatch(setView('rankings'))}>View Rankings</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="space-y-4 min-w-0">
          <div className="card overflow-hidden">
            ...
            <div className="card-body space-y-5">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    value={form.jobTitle}
                    onChange={(e) => dispatch(updateForm({ jobTitle: e.target.value }))}
              ...
              <div>
                ...
                <div
                  className="flex flex-wrap items-center gap-1.5 bg-white/[0.05] border border-white/[0.1] rounded-xl px-3 py-2.5 min-h-[48px] cursor-text"
                  onClick={() => document.getElementById('skillInput')?.focus()}
                >
                  {form.requiredSkills.map((s) => (
                    <span key={s} className="inline-flex items-center gap-1 bg-brand-500/20 text-brand-300 px-2.5 py-1 rounded-full text-xs">
                      {s}
                      <button
                        type="button"
                        onClick={() => dispatch(removeRequiredSkill(s))}
                  ...
                  <input
                    id="skillInput"
                    className="bg-transparent border-0 outline-none text-sm text-white placeholder-white/25 min-w-[120px] flex-1 py-1"
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => dispatch(updateForm({ minExperience: parseInt(e.target.value) || 0 }))}
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea
                  className="form-textarea min-h-[140px] rounded-xl"
                  value={form.description}
                  onChange={(e) => dispatch(updateForm({ description: e.target.value }))}
                  placeholder="Describe responsibilities, requirements, and nice-to-haves..."
                />
              </div>
            </div>
          </div>
          ...
          <div className="card overflow-hidden">
            <div className="card-header">
              <span className="card-title">AI Scoring Weights</span>
            </div>
            <div className="card-body space-y-5">
              {[
              ...
              <p className="text-xs text-white/30">
                Total: {form.scoringWeights.skills + form.scoringWeights.experience + form.scoringWeights.education}% (AI normalises automatically)
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 min-w-0">
          <div className="card">
            <div className="card-header">
              <span className="card-title">Add Candidates</span>
            </div>
            <div className="card-body space-y-4">
              <div className="grid grid-cols-3 gap-0.5 bg-white/[0.04] rounded-lg p-0.5">
                {(['json', 'pdf', 'manual'] as const).map((tab) => (
                  <button
                    key={tab}
                    type="button"
                    onClick={() => setActiveTab(tab)}
                    className={`py-1.5 text-xs rounded-md transition-all ${activeTab === tab ? 'bg-white/[0.09] text-white' : 'text-white/40 hover:text-white/60'}`}
                  >
              ...
              {activeTab === 'json' && (
                <div className="space-y-2">
                  <textarea
                    className="w-full bg-black/30 border border-white/[0.08] rounded-lg p-3.5 font-mono text-[11px] text-blue-200/70 leading-relaxed h-[180px] sm:h-[200px] resize-none outline-none focus:border-brand-500/30"
                    value={jsonInput}
                    onChange={(e) => setJsonInput(e.target.value)}
                    placeholder={'[\n  {\n    "firstName": "Jane",\n    "lastName": "Doe",\n    "headline": "Backend Engineer",\n    "skills": [...]\n  }\n]'}
                  />
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <button className="btn-ghost btn btn-sm w-full" onClick={parseJson}>
                      Parse JSON
                    </button>
                    <button className="btn-ghost btn btn-sm w-full" onClick={loadSamples}>
                      Load samples
                    </button>
                  </div>
                </div>
              ...
              {activeTab === 'pdf' && (
                <div>
                  <div
                    {...getRootProps()}
                    className={`border-2 border-dashed rounded-2xl p-6 sm:p-8 text-center cursor-pointer transition-all ${isDragActive ? 'border-brand-500/60 bg-brand-500/5' : 'border-white/15 hover:border-brand-500/40 hover:bg-brand-500/[0.03]'}`}
                  >
                    <input {...getInputProps()} />
                    <div className="w-11 h-11 bg-white/[0.06] rounded-2xl flex items-center justify-center mx-auto mb-3">
                      <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                        <path d="M9 2v9M6 5l3-3 3 3" stroke="rgba(255,255,255,0.5)" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                        <path d="M3 13v2h12v-2" stroke="rgba(255,255,255,0.3)" strokeWidth="1.4" strokeLinecap="round" />
                      </svg>
                    </div>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <div>
                      ...
                      <input className="form-input" value={manual.firstName} onChange={(e) => setManual({ ...manual, firstName: e.target.value })} placeholder="Jane" />
                    </div>
                    <div>
                      <label className="form-label">Last Name</label>
                      <input className="form-input" value={manual.lastName} onChange={(e) => setManual({ ...manual, lastName: e.target.value })} placeholder="Doe" />
                    </div>
                  </div>
                  <div>
                    <label className="form-label">Headline</label>
                    <input className="form-input" value={manual.headline} onChange={(e) => setManual({ ...manual, headline: e.target.value })} placeholder="Senior Node.js Engineer" />
                  </div>
                  <div>
                    <label className="form-label">Skills (comma-separated)</label>
                    <input className="form-input" value={manual.skills} onChange={(e) => setManual({ ...manual, skills: e.target.value })} placeholder="Node.js, React, MongoDB" />
                  </div>
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <div>
                      <label className="form-label">Years Experience</label>
                      <input className="form-input" type="number" value={manual.yoe} onChange={(e) => setManual({ ...manual, yoe: e.target.value })} />
                    </div>
                    <div>
                      <label className="form-label">Location</label>
                      <input className="form-input" value={manual.location} onChange={(e) => setManual({ ...manual, location: e.target.value })} placeholder="Kigali, Rwanda" />
                    </div>
                  </div>
                  <button className="btn-ghost btn w-full justify-center" onClick={addManual}>
                    + Add Candidate
                  </button>
                </div>
              )}
                  ...
                  <div className="space-y-1.5 max-h-[180px] overflow-y-auto pr-1">
                    {(staged as Array<{ firstName?: string; lastName?: string; skills?: unknown[] }>).map((c, i) => (
                      <div key={i} className="flex items-center gap-2 p-2 bg-white/[0.03] border border-white/[0.06] rounded-lg text-xs">
                        ...
                        <button onClick={() => dispatch(removeStagedCandidate(i))} className="text-white/30 hover:text-white/60 px-1">
                          x
                        </button>
                      </div>
                    ))}
          ...
          <button
            className="btn-primary btn w-full py-3.5 text-sm justify-center rounded-xl"
            ...
            {isLoading ? (
              ...
              <>
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <path d="M2 7h10M8 4l3 3-3 3" stroke="white" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                Run AI Screening
              </>
            )}
