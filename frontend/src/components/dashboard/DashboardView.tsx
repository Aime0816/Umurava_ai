...
export function DashboardView() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-7">
      {/* Stats row */}
      <div className="grid grid-cols-1 gap-3.5 mb-6 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <div key={stat.label} className="bg-white/[0.04] border border-white/[0.08] rounded-xl p-4">
            ...
            <div className="font-serif text-[22px] sm:text-[26px] font-light tracking-tight leading-none">{stat.value}</div>
            <div className="mt-2">
            ...
            {history.length === 0 && results.length === 0 ? (
              ...
              <div className="space-y-0">
                {history.slice(-4).reverse().map((s: Screening, i) => (
                  <div key={i} className="flex flex-wrap items-center gap-3 py-3 border-b border-white/[0.05] last:border-0">
                    ...
                    <span className="badge-info badge ml-auto sm:ml-0">
      ...
      {/* Top candidates */}
      <div className="card">
        <div className="card-header flex flex-col gap-2 sm:flex-row sm:items-center">
          <span className="card-title">Top Candidates - Latest Screening</span>
          {topFive.length > 0 && (
            <button className="btn-ghost btn btn-sm sm:ml-auto" onClick={() => dispatch(setView('rankings'))}>
              View all {'->'}
            </button>
          )}
        </div>
        <div className="card-body">
          {topFive.length === 0 ? (
            <EmptyState message="No results yet" sub="Complete a screening to see top candidates here">
              <button className="btn-primary btn mt-5" onClick={() => dispatch(setView('screening'))}>
                Start Screening
              </button>
            </EmptyState>
          ) : (
            <div className="space-y-2">
              {topFive.map((r: RankedCandidate, i) => (
                <div
                  key={i}
                  className="flex flex-col gap-3 rounded-lg border border-white/[0.05] bg-white/[0.02] p-3 transition-colors hover:border-white/10 sm:flex-row sm:items-center"
                >
                  <div
...
function EmptyState({ message, sub, children }: { message: string; sub: string; children?: React.ReactNode }) {
  return (
    <div className="text-center py-10 text-white/30">
      <div className="text-3xl mb-3 opacity-40">o</div>
